	...
	<div class="card">
		...
		<form @submit.prevent="createGlit">
			<textarea
				v-model="text"
				id="textarea"
				placeholder="Remember we hate KFC and messages that exceed 256 characters"
				maxlength="256"
			></textarea>
			<span class="counter">{{ text.length }} / 256</span>
			<div class="mb-3">
				<input
					class="form-control"
					id="formFile"
	...
	setup() {
		...
		const createGlit = async () => {
			...
			try {
				...
				errorMessage.value =
					"Failed to create glit. Remember: glits cannot be empty!";
